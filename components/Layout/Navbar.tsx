...
const Navbar = () => {
  ...
  return (
    <nav>
      <Box mx='auto' px="16px" py="40px" maxW="1260px">
        <Flex justify="space-between" align="center">
          <Image src={PhanesLogo} alt="Phanes Logo" height="33" />
          <HStack
          ...
          <Box _hover={{cursor: 'pointer'}} color="brand.lightText" display={{ base: "block", md: "none" }}>
            <GiHamburgerMenu fontSize='36px' onClick={() => {
              onOpen()
            }} />
          </Box>
        </Flex>
        <MobileNav data={routes} isOpen={isOpen} onClose={onClose}  />
      </Box>
    </nav>
  );
};
